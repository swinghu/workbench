package org.lambdata.techtestdrive.hadoop.joins.map;
...
public class MapSideJoinDriver {
	...
	public static void main(String[] args) throws Exception {
		...
		configureJob(firstSort, "firstSort", numReducers, jobOneInputPath,
				jobOneSortedPath, SortByKeyMapper.class, SortByKeyReducer.class);

		Job secondSort = Job.getInstance(getConfiguration(keyIndex, separator));
		configureJob(secondSort, "secondSort", numReducers, jobTwoInputPath,
				jobTwoSortedPath, SortByKeyMapper.class, SortByKeyReducer.class);

		Job mapJoin = Job.getInstance(getMapJoinConfiguration(separator,
				jobOneSortedPath, jobTwoSortedPath));
		configureJob(mapJoin, "mapJoin", 0, jobOneSortedPath + ","
				+ jobTwoSortedPath, joinJobOutPath, CombineValuesMapper.class,
				Reducer.class);
	...
	private static void configureJob(Job job, String jobName, int numReducers,
			String inPath, String outPath, Class<? extends Mapper> mapper,
	...
	private static Configuration getMapJoinConfiguration(String separator,
			String... paths) {
		...
		String joinExpression = CompositeInputFormat.compose("inner",
				KeyValueTextInputFormat.class, paths);
		config.set("mapred.join.expr", joinExpression);
		config.set("separator", separator);
		return config;
	}

	private static Configuration getConfiguration(String keyIndex,
			String separator) {
		Configuration config = new Configuration();
